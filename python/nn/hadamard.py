import python.tools.hadamard_utils as hadamard_utils
import torch
...
class OnlineHadamard(torch.nn.Module):
    def __init__(self, hadamard_dim, force_fp32=False):
        ...
        had_rem_dim, self.rem_dim = hadamard_utils.get_hadK(hadamard_dim)
    ...
    def forward(self, x):
        ...
        if self.fp32_had:
            x = x.float()
        x = hadamard_utils.matmul_hadU_cuda(x, self.had_rem_dim, self.rem_dim)
        x = x.to(x_dtype)
        return x
